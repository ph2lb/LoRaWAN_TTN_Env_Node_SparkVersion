...
static const PROGMEM u1_t NWKSKEY[16] = {  0x71, 0xB7, 0x2A, 0x65, 0x6F, 0xCC, 0xA3, 0x8C, 0xC3, 0x1B, 0x16, 0xD4, 0x0D, 0xEE, 0x2A, 0xE6 }; // LoRaWAN NwkSKey, network session key
static const u1_t PROGMEM APPSKEY[16] = { 0x4D, 0x47, 0x43, 0x71, 0xBD, 0x8B, 0x11, 0x6D, 0x16, 0x5C, 0x56, 0xCD, 0x9A, 0x0F, 0x9A, 0x98 }; // LoRaWAN AppSKey, application session key
static const u4_t DEVADDR = 0x260118B7 ; // LoRaWAN end-device address (DevAddr)
...
void updateEnvParameters()
{
  delay(1000);
  temp = bme.readTempC();
  pressure = bme.readFloatPressure()/100.0;    // returns PA so PA/100.0 = hPa (milliBar)
